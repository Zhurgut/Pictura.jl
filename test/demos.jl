...
@pictura begin
    ...
    @keypressed begin
        if KEY == ENTER
            save_frame(joinpath(@__DIR__, "testout.png"))
        end
    end
    ...
    @drawloop begin
        ...
        elseif m == 2
            Pictura.invert(Pictura.app.canvas, img)
        else
            Pictura.blur(img2, img)
            for i=1:20
                Pictura.blur(img3, img2)
                Pictura.blur(img2, img3)
            end
            image(img2)
        end
    end
...
@pictura begin
    ...
    @drawloop begin
        ...
        line(50, -100, 10, 100, infinite=true)
        rect(30, 30, 50, 100, corner_radius=rd)
        circle(-130, 30, 40)
    end
...
@pictura begin
    ...
    @drawloop begin
        ...
        for r=1:height(), c=1:width()
            ...
            P, Q, R = sim_noise3d(c*s, r*s, t)
            dPdx = (P - sim_noise3d(c*s+δ, r*s, t)[1]) / δ
            dQdy = (Q - sim_noise3d(c*s, r*s+δ, t)[2]) / δ
            dRdz = (R - sim_noise3d(c*s, r*s, t+δ)[3]) / δ
            div = dPdx + dRdz + dQdy
            ...
            pixels(Pictura.app.canvas)[r, c] = color(100div + 0.5)
        end
...
function draw_arrow(p1, p2)
    segment(p1, p2)
    orth = rotate(p2 - p1, π/2)
    # println(p1, ", ", p2)
    # println(p1 + 0.8*(p2-p1) + 0.2orth, ", ", p2)
    # println()
    segment(p1 + 0.8*(p2-p1) + 0.2orth, p2)
    segment(p1 + 0.8*(p2-p1) - 0.2orth, p2)
end
...
@pictura begin
    ...
    rd = color(255, 97, 97)
    blu = color(105, 161, 250)

    @drawloop begin
        background(0)
        for x=0:N:width(), y=0:N:height()
            p1 = Point(x, y)
            sp = S * p1
            dir = sim_noise3d(sp.x, sp.y, t)
            # dir = 1.5 .* curl_noise(sp.x, sp.y, t)
            # dir = 5 .* bitangent_noise(sp.x, sp.y, t)
            strokecolor(
                map(dir[3], -1, 1, (1/255) * red(rd), (1/255) * red(blu)),
                map(dir[3], -1, 1, (1/255) * green(rd), (1/255) * green(blu)),
                map(dir[3], -1, 1, (1/255) * blue(rd), (1/255) * blue(blu)),
            )
            draw_arrow(p1, p1 + 0.5N * Point(dir[1], dir[2]))
        end
        t += 0.01
    end
end
